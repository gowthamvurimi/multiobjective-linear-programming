...
for i in range(num_obj):
    ...
    list_obj_mat[i].append(-1/list_weights[i])

for i in list_mat:
    i = i.append(0)

list_obj = [0]*num_var + [1]

variables = method.calci(0, num_var+1, list_obj, num_obj+eq_l, eq_e, eq_g, list_obj_mat+list_mat, list_indi_ans+list_cons)



print("\n\n\n")
print(f"a pareto optimal sol: {[round(x, round_off) for x in variables[:-1]]}")

for i in range(num_obj):
    print(f"{i+1}th obj val({'max' if list_pb_type[i]==1 else 'min'}): ", end="")
    answer = np.linalg.multi_dot([variables[:-1], list_obj_mat[i][:-1]])
    print(f"{round(-answer if list_pb_type[i] else answer, round_off)} [individual = {-list_indi_ans[i] if list_pb_type[i]==1 else list_indi_ans[i]}, when values {list_indi_var[i]}]")